...
export default function SiderKiosk({ kioskList, handleKioskChange, kioskId }: ISiderKioskProps) {
    ...
    >
      ...
      <Menu
        onClick={(item) => {
          ...
          navigate(`/inventory/${item.key}`);
        }}
        defaultSelectedKeys={kioskId ? [kioskId] : [kioskList[0]?.deviceId]}
        mode="inline"
        items={kioskList.map((item) => getItem(item.kioskName, item.deviceId, <HddOutlined />))}
      />
    </Sider>
  );
}
